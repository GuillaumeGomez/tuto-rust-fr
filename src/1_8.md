...
```Rust
for (position, valeur) in (5..10).enumerate() {
...
```Shell
position = 0 et valeur = 5
position = 1 et valeur = 6
position = 2 et valeur = 7
position = 3 et valeur = 8
position = 4 et valeur = 9
```

__position__ vaut donc le nombre d'itérations effectuées à l'intérieur de la boucle tandis que __valeur__ prend successivement les valeurs du range `5..10`. Autre exemple :
