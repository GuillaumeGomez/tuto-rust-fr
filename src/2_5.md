...
Il est bon de noter que nous aurions pu remplacer la durée de vie (`'a`) du champ `content` par `'static`. Cependant faire ça nous aurait empêcher d'utiliser autre chose que des `String`s statiques, ce qui aurait été une grosse limitation.
