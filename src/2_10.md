...
Dans le cas présent, on récupère tout dans un slice qu'on récupère en le transformant en Vec. On pourrait aussi afficher tous les arguments un par un :
...
Vous aurez noté que j'ai remplacé les parenthèses par des accolades. Il aurait aussi été possible d'utiliser "{{ }}" ou même "[ ]". Après c'est plus une question de goût. Pourquoi "{{ }}" ? Tout simplement parce qu'ici nous avons besoin d'un bloc d'instruction. Si votre macro ne renvoie qu'une simple expression, vous n'en avez pas besoin.

###Débugger une macro
Je pense que vous vous en êtes vite rendus compte, les macros peuvent très vite devenir très complexe et débugger le tout ne semble pas être une mince affaire ! Sachez qu'il existe deux macros pouvant vous aider dans cette tâche :
 * __log_syntax!(...)__ : elle affiche les arguments sur la sortie standard __au moment de la compilation__.
 * __trace_macros!(true)__ : le compilateur affichera un message chaque qu'une macro sera utilisée. Utiliser __trace_macros!(false)__ pour la "désactiver".
...
Créer des macros, pouvoir s'en servir, c'est encore mieux ! Si vos macros sont déclarer dans un fichier à part (ce qui est une bonne chose !), il vous faudra ajouter cette ligne en haut du fichier où se trouvent vos macros :
...
Pour clôturer ce chapitre, je vous propose le code suivant qui permet de d'améliorer celui présenté dans le [chapitre sur la généricité](http://blog.guillaume-gomez.fr/blog/2/4.png) grâce à une macro :
