...
```Rust
fn addition(nb1: int, nb2: int) -> int {
    return nb1 + nb2;
}
```

Ne vous inquiétez pas si vous ne comprenez pas tout parfaitement, nous verrons les expressions dans le prochain chapitre. Un autre exemple pour illustrer cette différence :

```Rust
fn get_bigger(nb1: int, nb2: int) -> int {
    if nb1 > n2 {
        return nb1;
    }
    nb2
...
Cette façon n'est cependant pas recommandée en Rust, il aurait mieux valu écrire :
...
Une autre différence que certains d'entre vous auront peut-être noté (surtout ceux ayant déjà codé en C/C++) : je n'ai pas déclaré ma fonction addition et pourtant la fonction main l'a trouvé sans problème. Sachez juste que les déclarations de fonctions ne sont pas nécessaires en Rust (contrairement au C ou au C++ par-exemple).
...
Si vous pensez qu'elles ont quelque chose à voir avec celles que l'on peut trouver en C ou en C++, détrompez-vous ! Elles sont l'une des plus grandes forces de Rust, sont très complètes et permettent d'étendre les possibilités du langage. Par-contre, elles sont très complexes et seront le sujet d'un autre chapitre.
