...
impl Handler {
    ...
    pub fn do_something(&self) -> isize {
        unsafe { ffi::do_something(self.pointer) }
    }

    pub fn add_callback(&self, fonction: fn(isize, isize) -> isize) -> isize {
...
Voilà, vous devriez maintenant pouvoir vous en sortir avec ces bases. Nous avons vu là un callback, convertir une String entre C et Rust et nous avons surtout vu à quoi les __structures unitaires__ servaient !
