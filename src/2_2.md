...
Les noms des fonctions, par convention en __Rust__, doivent être écrits en [snake case](http://fr.wikipedia.org/wiki/Snake_case). Donc "MaFonction" est invalide, "ma_fonction" est correct.
...
On a implémenté le trait [__Drop__](https://doc.rust-lang.org/stable/std/ops/trait.Drop.html) à notre structure __Distance__. Quand l'objet est détruit, cette méthode est appelée. Je sais que cela ne vous dit pas ce qu'est un trait, mais pour plus d'explications, il va vous falloir lire le chapitre suivant !
