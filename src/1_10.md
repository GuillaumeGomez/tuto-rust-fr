...
```Rust
let t = Tuple(0, 2, false); // La structure tuple
...
let st = StructureTuple(1); // La structure "newtype"
```
...
On a implémenté le trait [__Drop__] à notre structure __Distance__. Quand l'objet est détruit, cette méthode est appelée. Je sais que cela ne vous dit pas ce qu'est un __trait__, mais nous y reviendrons dans la deuxième partie de ce cours.
